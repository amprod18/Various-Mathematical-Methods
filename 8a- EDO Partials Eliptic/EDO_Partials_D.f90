program EDO_Partials_D
    ...
    real(dp) :: Lx, Ly, h, T0(4), tinit(3), omega, heat_source_1, heat_source_2
    ...
    x_size = floor(Lx/h) + 1
    y_size = floor(Ly/h) + 1
    T0 = [0.5_dp, 17._dp, 11.2_dp, 25.3_dp] ! Borders: x0, 0y, xLy, Lxy
    ...
    do i = 1, size(tinit)
        do j = 1, 4
            if (j .eq. 1) then
                ...
                call initiateT(T0, x_size, y_size, T, tinit(i))
                call poisson_D(x_size, y_size, T, icontrol, verbose, make_gif, heat_source_1)
                write(1,*) ' '
                write(1,*) ' '
            else if (j .eq. 2) then
    ...
    icontrol = '9p_laplacian' ! jacobi; gauss_seidel; sobre_relaxacio; 9p_laplacian
    omega = 1.35_dp
    call initiateT(T0, x_size, y_size, T, tinit(1))
    call poisson_D(x_size, y_size, T, icontrol, verbose, make_gif, heat_source_1)
    do i = 1, x_size
        do j = 1, y_size
    ...
    icontrol = '9p_laplacian' ! jacobi; gauss_seidel; sobre_relaxacio; 9p_laplacian
    call initiateT(T0, x_size, y_size, T, tinit(1))
    call poisson_D(x_size, y_size, T, icontrol, verbose, make_gif, heat_source_2)
    do i = 1, x_size
        do j = 1, y_size
            write(1, *) (i-1)*h, (j-1)*h, T(i, j)
        end do
...
subroutine poisson_D(x_size, y_size, T, icontrol, verbose, make_gif, rho)
    use, intrinsic :: iso_fortran_env, only : dp => real64
    implicit none
    common/dades/h, Lx, Ly, omega
